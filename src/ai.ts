...
// The platform injects process.env.GEMINI_API_KEY into the browser environment safely
...
export async function generateAudit(url: string, industry: string): Promise<AuditResult> {
  ...
  const prompt = `
    Analyze this website: ${url} (Industry: ${industry})
    ...
    {
      "score": number (45-85),
      "estimatedRevenueLoss": number (monthly value),
      "executiveAnalysis": "string paragraph",
      "topIssues": [
        {
          "title": "string",
          ...
          "fix": "string",
          "whyItMatters": "string",
          "potentialImpactText": "string (e.g. +$2,450/mo)"
        }
      ], // Exactly 5
      "quickWins": ["string"], // Exactly 5
      "strategicRecommendations": ["string"], // Exactly 3
      "performanceMetrics": {
        "messaging": number,
        "trust": number,
        "performance": number,
...
export async function generateColdEmail(auditResult: AuditResult, scrapedData: any): Promise<EmailResult> {
  const prompt = `
    Generate a partner-level outreach email for ${scrapedData.url}.
